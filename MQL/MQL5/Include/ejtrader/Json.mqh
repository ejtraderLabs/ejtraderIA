...
bool CJAVal::Deserialize(char &js[], int slen, int &i)
{
    ...
    for (; i < slen; i++)
    {
        ...
        case '\r':
        case '\n':
        case ' ': // we are removing spaces from the nam
            i0 = i + 1;
            break;

        case '[': // start of the array. We create objects and retrieve them from js
        {
            i0 = i + 1;
            if (m_type != jtUNDEF) // if the value already has a type, then this is an error
            {
